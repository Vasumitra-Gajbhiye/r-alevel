...
export default function Sidebar({ subjectData }: { subjectData: SubjectDoc }) {
  ...
  const [openTopics, setOpenTopics] = useState<Record<string, boolean>>({});
  const [isMobileSheetOpen, setIsMobileSheetOpen] = useState(false);
  const [width, setWidth] = useState<number>(288); // default 72 * 4 = 288px
  ...
  // auto-expand chapter/topic with active subtopic
  useEffect(() => {
    if (!active) return;
    subjectData.chapters.forEach((chapter, cIdx) => {
      chapter.topics.forEach((topic) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newWidth = Math.min(Math.max(e.clientX, 200), 500); // limits: 200–500px
      setWidth(newWidth);
  ...
  return (
    <>
      ...
      <aside
        ref={sidebarRef}
        style={{ width }}
        className="hidden md:flex relative border-r bg-gray-50 h-[calc(100vh)] overflow-y-auto transition-[width] duration-100 ease-linear"
      >
        {SidebarContent}
        {/* Resizer handle */}
        <div
          onMouseDown={() => setIsResizing(true)}
          className={`absolute top-0 right-0 w-1 cursor-col-resize h-full bg-transparent hover:bg-gray-300 ${
