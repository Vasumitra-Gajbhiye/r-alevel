...
function blocksToMDX(blocks: EditorBlock[]) {
  return blocks
    .map((b) => {
      ...
      if (b.type === "Heading3") return `### ${b.data.text}\n`;
      if (b.type === "KeyPoints") {
        const titleAttr = b.data.title ? ` title="${b.data.title}"` : "";
        return `
        <KeyPoints color="${b.data.color}" variant="${
          b.data.variant
        }"${titleAttr}>
        ${b.data.items.map((item) => `- ${item}`).join("\n")}
        </KeyPoints>
        `;
      }
...
export default function PlaygroundPage() {
  ...
  function insertBlockAfter(id: string, newBlock: any) {
    setBlocks((p) => {
      const index = p.findIndex((b) => b.id === id);
      if (index === -1) return p;
      return [...p.slice(0, index + 1), newBlock, ...p.slice(index + 1)];
    });
  }

  function updateBlock(id: string, patch: Partial<EditorBlock["data"]>) {
    setBlocks((prev) =>
      prev.map((b) => {
        if (b.id !== id) return b;
        ...
        switch (b.type) {
          ...
          case "BlogParagraph":
            return {
              ...b,
              data: {
                text: "text" in patch ? patch.text! : b.data.text,
              },
            };
          ...
          case "Callout":
            return {
              ...
              data: {
                ...b.data,
                ...(patch as Partial<typeof b.data>),
              },
            };
  ...
  return (
    <div className="h-screen overflow-hidden grid grid-cols-2">
      ...
      <div className="h-full overflow-y-auto px-8 py-6 scrollbar-minimal">
            ...
            {metaOpen && (
              ...
              >
                ...
                <input
                  ...
                  value={metadata.tag || ""}
                  onChange={(e) =>
                    setMetadata({ ...metadata, tag: e.target.value })
                  }
                  className="w-full"
                />
        ...
        {/* ---- Add blocks ---- */}
        <div className="grid grid-cols-4 gap-x-2 gap-y-2 mb-6">
          {BLOCK_REGISTRY.map((block) => (
            <button
              key={block.type}
              onClick={() => setBlocks((prev) => [...prev, block.create()])}
          ...
          >
            {blocks.map((block) => (
              <SortableBlock
                ...
                onAddBelow={(newBlock) => insertBlockAfter(block.id, newBlock)}
              >
                  ...
                  <div className="border rounded-xl p-4 space-y-3">
                    ...
                    <label className="flex items-center gap-2 text-sm">
                      <input
                        ...
                        onChange={(e) =>
                          updateBlock(block.id, { ordered: e.target.checked })
                        }
                      />
                      Ordered list
                    </label>
...
- Third point`}
                ...
                {block.type === "Video" && (
                  <Video
                    ...
                    showInput
                    showPreview={false}
                    onChange={(value) => updateBlock(block.id, { src: value })}
                  />
                )}
              </SortableBlock>
            ))}
      ...
      {/* ================= RIGHT: PREVIEW ================= */}
      <div className="h-full overflow-y-auto px-10 py-6 border-l border-slate-200 scrollbar-minimal">
        <BlogPostLayout metadata={metadata} showToc={false}>
          {blocks.map((block) => {
            switch (block.type) {
              case "BlogHeading":
                return (
                  <BlogHeading key={block.id}>{block.data.text}</BlogHeading>
                );
